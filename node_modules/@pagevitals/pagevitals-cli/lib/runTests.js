...
async function runTests(options, apiToken, details) {
    ...
    if (options.page?.length) {
        options.pages = options.page.map(p => {
...
function formatTestResults(data) {
    ...
    const headers = ['Test ID', 'Page', 'Device', 'Performance', 'Accessibility', 'Best Practices', 'SEO'];
    const columnWidths = [20, 8, 11, 13, 14, 5];
    const columnTypes = ["string", "string", "string", "score", "score", "score", "score"];
    const rows = result.list.map(test => [
        test.id,
        test.page.alias + (test.status === "failed" ? " (failed)" : ""),
        test.device,
